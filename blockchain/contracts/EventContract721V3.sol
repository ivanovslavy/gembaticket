...
/// @title EventContract721V3 — ERC721 ticket contract with signature-based claiming
...
///         `claimTicketFor(recipient, ...)`, pay the gas itself, and mint the NFT to the holder
///         for free. The mintSigner signature binds the recipient, so even a compromised relayer
///         cannot redirect a mint. The original `claimTicket` flow is preserved UNCHANGED.
...
contract EventContract721V3 is
    Initializable,
    ERC721Upgradeable,
    ReentrancyGuardUpgradeable
{
    ...
    /// @dev Signature = mintSigner signs keccak256(contract, claimHash, recipient).
    ...
    ///      mint to another wallet; (3) usedClaims prevents double-mint (shared with claimTicket,
    ///      so a ticket can never be claimed by both paths); (4) nonReentrant; (5) supply guards.
    /// @param _recipient  Wallet that receives the NFT (the ticket holder).
    /// @param _claimHash  Unique claim identifier (keccak256 of ticket UUID).
    /// @param _signature  MintSigner's ECDSA signature over (contract, claimHash, recipient).
    function claimTicketFor(
        ...
        // Verify mintSigner signed (contract, claimHash, recipient) — recipient is bound.
        bytes32 messageHash = keccak256(abi.encodePacked(
            address(this), _claimHash, _recipient
        ));
        bytes32 ethSigned = _toEthSignedMessageHash(messageHash);
        ...
        emit TicketClaimed(_recipient, tokenId, _claimHash);     // indexer compatibility
        emit GaslessClaim(_recipient, tokenId, _claimHash, msg.sender);  // audit trail
    }
    ...
    function increaseSupply(uint256 _additional) external onlyOwnerOrOperator {
    ...
    function setTokenURI(uint256 _tokenId, string calldata _uri) external onlyOwnerOrOperator {
        _tokenURIs[_tokenId] = _uri;
    }

    /// @notice Update base URI (fallback for tokens without custom URI).
    function updateBaseURI(string calldata _newBaseURI) external onlyOwnerOrOperator {
        _baseTokenURI = _newBaseURI;
        emit BaseURIUpdated(_newBaseURI);
    }
    ...
    /// @dev Operator can perform day-to-day ops (supply, sale, URIs) on owner's behalf.
