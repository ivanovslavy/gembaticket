...
- **Chain support**: **GembaBlockchain** — GembaTicket's own EVM-compatible Layer-1 with ~0 gas fees (testnet `821207` live, mainnet `821206` rolling out). RPC is a fallback pool over GembaBlockchain endpoints (`rpc1`/`rpc2`/`rpc3.gembascan.io`) in `rpcEndpoints.js`. (The platform was originally planned for Polygon; it now runs on GembaBlockchain.)
